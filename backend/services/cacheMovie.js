...
const cacheMovie = async (movieResult) => {
    try {
        ...
        const movieInsert = await db.query("INSERT INTO movie (id, title, poster_path, release_date, overview, budget, runtime, revenue) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) ON CONFLICT (id) DO NOTHING", 
        ...
        if (movieInsert.rowCount === 0) {
            const check = await db.query('SELECT id FROM movie WHERE id = $1', [movieResult.id]);
            if (check.rowCount === 0) {
                throw new Error(`Movie ${movieResult.id} could not be found in movie table. Aborting cacheMovie`);
            }
        }
        await addMovieGenres(movieResult, db);
        await addMovieCast(movieResult.id, db);
